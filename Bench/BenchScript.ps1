do {
  $Input = 0
  while ( $Input -lt 1 -or $Input -gt 5) {
    Write-Host "1. Complete Bench Cycle"
    Write-Host "2. Remove Bloatware"
    Write-Host "3. Set Default Settings"
	Write-Host "4. Install Java"
    Write-Host "5. Install Adobe Reader"

    $Input = Read-Host "Please choose an option" -ErrorAction Stop

    switch ($Input) {
      1{.\hpdesktop.ps1}
      2{Write-Host "Uninstalling Bloatware"
      ...
      3{import-module .\switchuaclevel.psm1
